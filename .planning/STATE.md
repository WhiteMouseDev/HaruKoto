...
- N4 CH07 rollout follow-up - HN4-027 through HN4-031 now have staged official lesson JSON and official TTS manifest targets, but the file is not in the default API seed registry and the five `lesson-human-review` rows remain `PENDING`. Next wave should generate TTS, run delegated audio QA, apply an approved configured DB seed path, run API/mobile smoke, then close `lessons:review:gate` before learner-facing exposure.
...
Latest stopped at: N4 CH07 official lesson JSON staging is prepared for HN4-027 through HN4-031 and CH07 official TTS manifest targets are derived. CH07 is not in the default API seed registry and `lesson-human-review` keeps the five CH07 rows `PENDING` to prevent rollout-gate clearance before TTS/audio/seed/UAT work.
Stopped at: N4 CH01 through CH06 configured seed, TTS coverage, delegated audio QA, URL-inclusive rollout preflight, and broad exposure decision are green. CH07 has official lesson staging data ready for the next operational wave: generate TTS, run delegated audio QA, seed/smoke the configured target, run mobile UAT, then close `lessons:review:gate`.
