...
function KakaoCompleteInner() {
  ...
  useEffect(() => {
    ...
    if (!idToken) {
      router.replace('/login?error=kakao_no_id_token');
      return;
    }
    ...
    async function handleSignIn(token: string) {
      ...
      if (error) {
        console.error('Kakao signInWithIdToken failed:', error.message);
        router.replace('/login?error=kakao_signin_failed');
        return;
      }

      // Check/create user in DB — same logic as /auth/callback/route.ts
      const {
        data: { user },
