...
from app.services.quiz_policy import apply_srs_update, calculate_smart_distribution
...
def test_distribution_basic():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=10, retry_due=3)
...
def test_distribution_no_review_no_retry():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=0, retry_due=0)
...
def test_distribution_retry_capped_at_20_percent():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=0, retry_due=100)
...
def test_distribution_review_capped_at_75_percent():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=100, retry_due=0)
...
def test_distribution_mild_debt():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=25, retry_due=0)
...
def test_distribution_min_new_always_guaranteed():
    ...
    result = calculate_smart_distribution(daily_goal=20, review_due=500, retry_due=50)
    assert result["new"] >= 2


def test_distribution_small_goal():
    """Small daily goal still distributes sensibly."""
    result = calculate_smart_distribution(daily_goal=5, review_due=3, retry_due=1)
...
def test_srs_correct_first_time():
    ...
    apply_srs_update(p, is_correct=True, time_spent_seconds=5, now=now)
    assert p.streak == 1
...
def test_srs_instant_bonus():
    ...
    now = datetime.now(UTC)
    apply_srs_update(p, is_correct=True, time_spent_seconds=2, now=now)
...
def test_srs_slow_correct_quality_3():
    ...
    p = _make_progress(streak=2, interval=3, ease_factor=2.5, correct_count=2)
    now = datetime.now(UTC)
    apply_srs_update(p, is_correct=True, time_spent_seconds=15, now=now)
...
def test_srs_incorrect_lapse_multiplier():
    ...
    now = datetime.now(UTC)
    apply_srs_update(p, is_correct=False, time_spent_seconds=5, now=now)
    assert p.streak == 0
