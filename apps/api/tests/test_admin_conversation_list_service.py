...
@pytest.mark.asyncio
async def test_list_admin_conversation_returns_normalized_paginated_items() -> None:
    ...
    db = _FakeDb([_ScalarResult(21), _ScalarsResult(rows)])
    ...
    assert [item.title for item in result.items] == ["카페 주문", "출장 회의"]
    assert result.items[0].category == "DAILY"
    assert result.items[0].jlpt_level is None
    assert result.items[0].review_status == "needs_review"
    assert result.items[1].category == "BUSINESS"
    assert result.items[1].review_status == "approved"
    assert db.execute_calls == 2


@pytest.mark.asyncio
async def test_list_admin_conversation_uses_one_total_page_for_empty_result() -> None:
