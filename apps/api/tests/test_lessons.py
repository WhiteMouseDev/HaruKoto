...
@pytest.mark.asyncio
async def test_lesson_question_prompt_tts_uses_server_lesson_prompt(client, mock_user):
    """POST /lessons/{id}/questions/{order}/tts resolves prompt server-side."""
    from app.main import app
    ...
    async def fake_generate_tts(text: str):
        assert text == "厚いの意味は何ですか。"
        return MagicMock(audio=b"mp3", provider="elevenlabs", model="eleven_multilingual_v2")

    async def fake_upload(path: str, audio: bytes):
        assert path == f"tts/lesson/{lesson_id}/question-1.mp3"
        assert audio == b"mp3"
        return "https://cdn.example.com/lesson-question.mp3"
    ...
    with (
        ...
        response = await client.post(f"/api/v1/lessons/{lesson_id}/questions/1/tts")

    assert response.status_code == 200
    assert response.json() == {"audioUrl": "https://cdn.example.com/lesson-question.mp3"}
    assert mock_session.add.call_args.args[0].target_type == "lesson_question_prompt"
    assert mock_session.add.call_args.args[0].text == "厚いの意味は何ですか。"
    mock_session.commit.assert_awaited_once()
