...
test('renders cloze quiz detail with audit history and submits approve action', async ({
  page,
}) => {
  const api = await mockAdminApi(page);

  await page.goto('/quiz/cloze-1?type=cloze');
  ...
  await expect(page.getByLabel('訳')).toHaveValue(
    '나는 매일 아침 커피를 마신다.'
  );
  await expect(page.getByLabel('正解')).toHaveValue('飲みます');
  await expect(page.getByLabel('選択肢（JSON）')).toHaveValue(/食べます/);
  await expect(page.getByLabel('説明')).toHaveValue(
    '飲み物には「飲みます」を使います。'
  );
  await expect(page.getByText('quiz-reviewer@harukoto.test')).toBeVisible();
...
test('renders sentence arrange quiz detail and submits reject action', async ({
  page,
}) => {
  const api = await mockAdminApi(page);

  await page.goto('/quiz/arrange-1?type=sentence_arrange');

  await expect(
    page.getByRole('heading', { name: 'クイズを編集' })
  ).toBeVisible();
  await expect(page.getByLabel('韓国語文')).toHaveValue(
    '도서관에서 일본어를 공부합니다.'
  );
  await expect(page.getByLabel('日本語文')).toHaveValue(
    '図書館で日本語を勉強します。'
  );
  await expect(page.getByLabel('トークン（JSON）')).toHaveValue(/図書館/);
  await expect(page.getByLabel('説明')).toHaveValue(
    '場所を表す「で」を使います。'
  );
  await expect(page.getByText('arrange-reviewer@harukoto.test')).toBeVisible();
  await expect(
    page.getByRole('button', { name: '再生', exact: true })
  ).toBeVisible();
