...
import { buildCandidatePacket } from './prepare-lesson-seed-candidate-review.mjs';

const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url));
const PACKAGE_DIR = join(SCRIPT_DIR, '..');
const REVIEW_DIR = join(PACKAGE_DIR, 'data', 'curriculum', 'lesson-seed-candidate-review');
...
function usage() {
  return 'Usage: node scripts/validate-lesson-seed-candidate-review.mjs [--in <reviewDir>]';
}
...
function listReviewFiles(reviewDir, issues) {
  if (!existsSync(reviewDir)) {
    addIssue(issues, 'FAIL', 'lesson seed candidate review', `Missing review directory: ${reviewDir}`);
    return [];
  }
  ...
  if (files.length === 0) {
    addIssue(issues, 'FAIL', 'lesson seed candidate review', `No review packet JSON files found in ${reviewDir}`);
  }
...
function validatePacket(packet, fileName, issues) {
  ...
  if (packet.reviewKind !== 'lesson_seed_candidate_curriculum_review') {
    addIssue(issues, 'FAIL', fileName, 'reviewKind must be lesson_seed_candidate_curriculum_review.');
  }
  if (!LEVELS.has(packet.level)) {
    addIssue(issues, 'FAIL', fileName, 'level must be one of N5, N4, N3, N2, N1.');
  }
  if (packet.includePromoted !== false) {
    addIssue(issues, 'FAIL', fileName, 'includePromoted must be false for source-controlled candidate review packets.');
  }

  const rowCount = validateReviewRows(packet, fileName, issues);
  if (LEVELS.has(packet.level)) {
    const candidateIds = (packet.reviewRows ?? []).map((row) => row?.candidateId).filter((candidateId) => candidateId);
    const expectedPacket = buildCandidatePacket(packet.level, { candidateIds });
    if (!packetsMatchIgnoringReviewerState(packet, expectedPacket)) {
      addIssue(
        issues,
        'FAIL',
        fileName,
        `Packet drifted from candidate/TTS source data. Run candidates:review:prepare -- --level ${packet.level}.`,
      );
    }
  }
...
function main() {
  ...
  console.log('Lesson seed candidate review validation summary');
  ...
  console.log('Lesson seed candidate review validation passed.');
}
