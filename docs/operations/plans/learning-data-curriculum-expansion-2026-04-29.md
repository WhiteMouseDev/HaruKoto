...
- 프로덕션 DB에는 별도 seed 실행 전까지 반영되지 않는다.
- N4 TTS 대상 생성, admin 검수 화면, mobile target-runtime 학습 완료 UAT는 후속 작업이다.
...
3. 프로덕션 DB seed 실행 전 staging 또는 disposable DB에서 N4 seed dry-run을 수행한다.
