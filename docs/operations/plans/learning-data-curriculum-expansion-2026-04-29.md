...
> 작성일: 2026-04-29
> 상태: Draft — N4 pilot seed promotion in progress
...
### 8.1 N4 Pilot Official Seed Promotion

2026-05-11 기준 Wave 2 N4 foundation 후보 중 첫 10개 레슨을 공식 lesson seed 파일로 승격했다. 이 승격은 N4 전체 커버리지 완료가 아니라, N4 레벨을 선택한 사용자가 실제 pilot 학습 경로에 진입할 수 있는 최소 배치를 여는 작업이다.

공식 seed source는 다음 두 파일이다.

| File | Chapter | Lessons | Runtime status |
|---|---|---:|---|
| `packages/database/data/lessons/n4/ch01-core-directions-and-judgment.json` | `N4-CH01` 핵심 지시와 판단 | 5 | `PILOT` publishable |
| `packages/database/data/lessons/n4/ch02-reasons-conditions-and-intent.json` | `N4-CH02` 이유, 조건, 의도 표현 | 5 | `PILOT` publishable |

API seed registry는 `CONTENT_FILES_BY_LEVEL["N4"]`에 위 두 파일을 명시한다. `meta.status = PILOT`은 lesson seed 정책상 `is_published=true`로 매핑되어, seed 명령 실행 시 N4 pilot 레슨으로 노출될 수 있다.

2026-05-11 운영 업데이트: N4 pilot seed는 현재 configured API DB target에 적용됐고, `lessons --check --level N4`는 2개 chapter, 10개 lesson, 0 missing, 0 mismatch로 통과했다. 인증된 runtime API smoke도 N4 lesson list/detail을 검증했다. 첫 N4 lesson detail은 4개 script line, 5개 question, 5개 vocabulary item, 1개 grammar item을 반환했고, PR #77 이후 lesson detail 응답의 `correctAnswer`/`correctOrder`는 모두 redacted 상태다.

2026-05-12 운영 업데이트: delegated AI curriculum review, official lesson-seed TTS target scope, target-runtime mobile happy path, and target-runtime wrong-answer retry UAT are complete for the first 10 N4 pilot lessons. The learner rollout decision is LIMITED GO for controlled pilot exposure only. Broad/full N4 rollout remains HOLD until pilot feedback, full N4 coverage planning, native-speaker review when available, and full lesson-seed TTS generation/audio QA are handled. See `docs/operations/plans/n4-pilot-learner-rollout-decision-2026-05-12.md`.

ASSUMPTION: 이 문서의 "configured API DB target"은 seed와 smoke 실행 시점의 `apps/api` runtime 환경이 바라본 DB를 뜻한다. 별도 production rollout 승인은 human curriculum review, TTS scope, mobile UAT 결과를 보고 결정한다.

이 승격으로 충족된 조건:

- seed file shape가 기존 lesson runtime의 `reading_script`, `questions`, vocabulary/grammar reference 계약을 따른다.
- N4 pilot batch는 총 10개 레슨과 50개 문항으로 제한한다.
- PDF 자료는 주제 커버리지와 난이도 순서 참고로만 사용하고, 제품에 들어가는 예문/대화/문항은 HaruKoto 신규 콘텐츠만 사용한다.
- 기존 N5 default seed source는 유지한다.

운영 게이트 상태:

- Delegated AI curriculum review는 닫혔다. Native-speaker human review는 가능해지는 시점에 broad rollout 전 상위 검수로 다시 다룬다.
- Official lesson-seed TTS target scope는 닫혔다. 실제 batch TTS 생성과 audio QA는 broad rollout 전 별도 게이트로 남긴다.
- Mobile target-runtime lesson list/result/SRS/wrong-answer retry UAT는 닫혔다.
...
1. N4 pilot seed 10개를 admin/human curriculum review에 올린다.
2. N4 pilot seed의 TTS 대상 생성 범위와 batch review 순서를 확정한다.
3. 배포 환경별 DB target이 configured API DB target과 다르면, 해당 target에서 N4 seed check/dry-run을 별도로 수행한다.
4. N4 레벨 선택 후 lesson list, quiz result, SRS 등록이 target runtime에서 정상 동작하는지 UAT한다.
5. PDF 001-102 topic inventory와 기존 grammar/topic mapping을 계속 확장한다.
6. `covered / partial / missing / deferred` 커버리지 리포트를 만들고 N4 lesson 11+ wave를 분리한다.
...
10. N4 lesson 11+ wave는 coverage report와 human review 결과를 기준으로 별도 seed batch로 분리한다.
11. API/mobile/admin의 N4 level-local lesson 번호와 review 상태 표시는 target-runtime UAT와 admin/human review에서 다시 확인한다.
