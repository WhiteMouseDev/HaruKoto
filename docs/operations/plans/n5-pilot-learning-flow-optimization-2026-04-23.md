...
| 모바일 UAT 재실행 | 부분 완료 | Lesson 2/3 실제 완료 플로우와 핵심 계측 PASS, 결과 화면 SRS 안내와 review CTA는 위젯 계약 테스트 보강 |
...
- `review_cta_clicked`는 실계정에서 복습 대기 카드가 미노출되어 end-to-end 탭 검증을 하지 못했다.
  대신 `StudyPage` 위젯 테스트로 `totalDue > 0`일 때 `복습 시작` 탭 이벤트 계약을 검증한다.
...
| review_cta_clicked | 계약 확인 / 실계정 미확인 | 복습 CTA 미노출. 위젯 테스트로 due card 탭 이벤트 속성 검증 |
