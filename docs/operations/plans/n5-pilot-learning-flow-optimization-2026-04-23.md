...
lesson JSON의 `meta.status`는 파일럿 동안 아래 중 하나로 정리한다.

- 권장: `PILOT`
- 대안: `DRAFT` 유지 + seed 스크립트에서 `--publish-draft` 같은 의도를 명시

현재처럼 `DRAFT` 데이터를 무조건 `is_published=True`로 올리는 방식은 운영 의미가 불명확하다.
...
1. `apps/api/app/seeds/lessons.py`를 공식 파일럿 seed로 문서화한다.
2. 개발/스테이징 setup 문서에 vocabulary/grammar seed 이후 lesson seed 실행 순서를 추가한다.
...
초기에는 분석 이벤트 구현이 없더라도 수동 UAT 체크리스트로 동일 항목을 기록한다.

## 8. 리스크와 대응

| 리스크 | 대응 |
|--------|------|
| 데이터가 DRAFT인데 publish됨 | `PILOT` 상태 또는 explicit publish flag 도입 |
